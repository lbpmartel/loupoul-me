...
Salut 👋 J'ai créé la React Testing Library parce que je n'étais pas satisfait de la réalité des tests du moment. Ça a grossi vers une librairie de tests sur le DOM, et maintenant nous avons des implémentations de la librairie de test (« wrappers ») pour les frameworks Javascript les plus populaires, ainsi que pour les outils de test qui visent les éléments du DOM (et même pour ceux qui visent d'autres types d'éléments).

Nous avons apporté différents changements à l'API depuis le debut, et nous avons découvert des façons de faire sous-optimales. Malgré nos efforts à documenter les « meilleures façons de faire » pour utiliser les outils que l'on propose, je vois encore des articles de blog et des tests écrits en suivant ces façons de faire sous-optimales, et j'aimerais passer en revue quelques une d'entre elles, expliquer pourquoi elles ne sont pas idéales et de quelle façon ameliorer vos tests pour eviter ces pièges de conception.
