...
Le nom `wrapper` est un « déchet résiduel » d'une utilisation de Enzyme et nous n'avons pas besoin de ça ici. La valeur de retour de `render` n'enveloppe rien du tout. C'est simplement une collection d'outils utilitaires que (merci au prochain point) vous ne devriez pas avoir à utiliser fréquemment de toute façon.

**Conseil: déstructurez ce dont vous avez besoin de l'appel de `render` ou appelez la variable `view`.**
