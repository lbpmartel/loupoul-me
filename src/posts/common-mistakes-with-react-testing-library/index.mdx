...
title: "Erreurs fréquentes avec React Testing Library"
date: "2020-05-04"
translation-date: "2021-07-10"
author: "Kent C. Dodds"
translator: Louis-Bernard Poulin Martel
description: "Quelques erreurs que les gens font fréquemment en utilisant React Testing Library."
categories:
  - "testing"
keywords:
  - "react"
...
###### Version originale : [«Common mistakes with React Testing Library»](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library) de Kent C. Dodds

Salut 👋 J'ai créé la React Testing Library parce que je n'étais pas satisfait de la réalité des tests du moment.

Ça a grossi vers une librairie de tests sur le DOM, et maintenant nous avons des implémentations de la librairie de test (« wrappers ») pour les frameworks Javascript les plus populaires, ainsi que pour les outils de test qui visent les éléments du DOM (et même pour ceux qui visent d'autres types d'éléments).

Nous avons apporté différents changements à l'API depuis le début, et nous avons découvert des façons de faire sous-optimales. Malgré nos efforts à documenter les « meilleures façons de faire » pour utiliser les outils que l'on propose, je vois encore des articles de blog et des tests écrits en suivant ces façons de faire sous-optimales, et j'aimerais passer en revue quelques une d'entre elles, expliquer pourquoi elles ne sont pas idéales et de quelle façon ameliorer vos tests pour eviter ces pièges de conception.

> Note: Je vais classifier chacune d'elles selon leur ordre d'importance :
>
> - basse: ceci n'est principalement que mon opinion, soyez à l'aise de les ignorer et vous n'aurez probablement pas de problème.
>
> - moyenne: vous pourriez rencontrer des bogues, perdre de la certitude ou faire du travail en double.
>
> - haute: écoutez définitivement ce conseil! Vous manqueriez fort probablement de certitude dans vos tests ou pourriez avoor des tests problématiques.

## Ne pas utiliser l'extension Testing Library ESLint

> Importance: moyenne

Si vous voulez éviter plusieurs de ces erreurs communes, alors cette extension ESLint officielle pourrait grandement vous aider :

- [eslint-plugin-testing-library](https://github.com/testing-library/eslint-plugin-testing-library)
- [eslint-plugin-jest-dom](https://github.com/testing-library/eslint-plugin-jest-dom)

**Conseil: Installez et utilisez l'extension ESLint pour la Testing Library.**

## Utiliser `wrapper` comme nom de variable pour la valeur de retour de `render`
...
Le nom `wrapper` est un « déchet résiduel » d'une utilisation de Enzyme et nous navons pas besoin de ça ici. La valeur de retour de `render` n'enveloppe rien du tout. C'est simplement une collection d'outils utilitaires que (merci au prochain point) vous ne devriez pas avoir à utiliser fréquemment de toute façon.

**Conseil: destructurez ce dont vous avez besoin de l'appel de `render` ou appelez la variable `view`.**

## L'utilisation de `cleanup`
...
Depuis déjà longtemps, `cleanup` s'effectue automatiquement (supporté par les principaux frameworks de test) et vous n'avez plus besoin de vous en soucier.
[Apprenez-en plus](https://testing-library.com/docs/react-testing-library/api#cleanup).

**Conseil: N'utilisez pas `cleanup`**

## Ne pas utliser `screen`
...
`screen`
[a été ajouté dans la version v6.11.0 de DOM Testing Library](https://github.com/testing-library/dom-testing-library/releases/tag/v6.11.0)
(ce qui veut dire que vous y avez accès avec `@testing-library/react@>=9`). Ça vient du même import que celui pour lequel on obtient `render` :

```javascript
import { render, screen } from "@testing-library/react"
```

Le bénéfice d'utiliser `screen` est que vous n'aurez oplus besoin de garder la déstructuration de l'appel
à `render` à jour lorsque vous ajouterez ou supprimerez les requêtes dont vous aurez besoin.
Vous n'avez qu'à écrire `screen` et laisse la magie de l'autocomplete de l'éditeur faire le travail.

La seule exception à ceci
